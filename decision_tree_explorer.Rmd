...
DATA_DIR <- "models"  # "../newera/newera_trees" # "test_data"
...
simplify_rule <- function(rule_vector){
  rule_hash <- new.env(hash = TRUE)
  
  get_rule_key_value <- function(rule){
    # rule key is feature plus comparison operator
    SEP <- ' '
    rule_atoms <- strsplit(rule, SEP)[[1]]
    key <- paste(rule_atoms[1:2], collapse=SEP)
    value <- paste(rule_atoms[-c(1,2)], collapse=SEP)
    if (substr(value, 1, 2) == "c(")
      value <- eval(parse(text=value))
    list(key=key, value=value)
  }
  
  lapply(rule_vector, get_rule_key_value)
  
  for (rule in rule_vector){
    key_val <- get_rule_key_value(rule)
    key <- key_val$key
    value <- key_val$value
    if(is.null(rule_hash[[key]])) rule_hash[[key]] <- list()
    rule_hash[[key]] <- append(rule_hash[[key]], list(value))
  }
  
  simplified_rules_list <- mget(ls(rule_hash), rule_hash)
  
  srl <- lapply(setNames(nm=names(simplified_rules_list)), function(rname){
    rule_value_list <- simplified_rules_list[[rname]]
    if (grepl('%in%', rname)){
      rule <- Reduce(intersect, rule_value_list)
    } else {
      rule <- tail(rule_value_list, 1)[[1]]
    }
    rule
  })
...
get_simplified_rule_table <- function(original_rule){
  ...
  data.frame(rule=simplify_rule(rule_vector))
}
...
server <- function(input, output) {
  ...
  output$rule_table <- renderTable({
    get_clicks()
    prunedTree <- PRUNED_TREE
    original_rule <- unlist(get_node_info(SELECTED_NODE, prunedTree, input$dataset)$rule)
    get_simplified_rule_table(original_rule)
  })
}
